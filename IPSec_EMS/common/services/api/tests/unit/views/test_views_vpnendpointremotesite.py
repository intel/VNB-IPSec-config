...
# VPNEndpointRemoteSite test record
VPNENDPOINTREMOTESITE_RECORD = {
    # 'id':
    'name': 'remotesite1',
    'description': 'myremotesite1',
    'peer_address': '10.1.63.21',
    'peer_cidrs': ['10.1.65.0/24'],
    # 'vpncertificate_id':
}
...
class TestVPNEndpointRemoteSiteCRUD(APITestCase):
    ...
    def setUp(self):
        ...
        if self._testMethodName.startswith('test_post') or \
            ...
            self.url = reverse('vpnendpointremotesites_list', kwargs={
                'version': 'v1',
                'namespace': 'main'
                })
...
class TestVPNEndpointRemoteSiteNotFound(APITestCase):
    """Test case for GET, PUT & DELETE operations on
     VPNEndpointRemoteSite with no record"""

    def setUp(self):
        self._uuid = generate_uuid()
        self._url = reverse('vpnendpointremotesites_list', kwargs={
            'version': 'v1',
            'namespace': 'main'
            })
    ...
    def test_list_with_no_records(self):
        """Test case to list all VPNEndpointRemoteSite with no records
         present."""
    ...
    def test_get_with_invalid_id(self):
        """Test case to show or get an VPNEndpointRemoteSite with invalid id"""
        self._url = self._url_prefix + self._uuid + '/'
        response = self.client.get(self._url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_put_with_invalid_id(self):
        """Test case to update an VPNEndpointRemoteSite with invalid id"""
        self._url = self._url_prefix + self._uuid + '/'
        self._update_value = {'description': 'myremotesite2'}
        response = self.client.put(self._url, self._update_value,
                                   format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_delete_with_invalid_id(self):
        """Test case to delete an VPNEndpointRemoteSite with invalid id"""
        self._url = self._url_prefix + self._uuid + '/'
        response = self.client.delete(self._url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)


class TestVPNEndpointRemoteSiteBadRequest(APITestCase):
    """Test case for POST & PUT operations on VPNEndpointRemoteSite with invalid
     record"""
    ...
    def setUp(self):
        ...
        if not self._testMethodName.startswith('test_post_'):
            ...
            self.vpnendpointremotesite = (
                VPNEndpointRemoteSite(**self._data).save())
